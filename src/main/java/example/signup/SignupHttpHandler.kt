...
class SignupHttpHandler(private val transactor: Transactor<SignupBook>) : HttpHandler {
        ...
        when (exchange.requestMethod) {
            ...
            HttpMethod.POST -> {
                when (sheet) {
                    ...
                    is Full -> {
                        sendResponse(exchange, CONFLICT, "session is full")
                    }
                    
                    is Closed -> {
                        sendSessionClosed(exchange)
                    }
                }
            }
    ...
    private fun sendSessionClosed(exchange: HttpExchange) {
        sendResponse(exchange, CONFLICT, "session is closed")
    }
